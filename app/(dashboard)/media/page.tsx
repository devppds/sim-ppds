...
import { Plus, Search, RefreshCw, X, Pencil, Trash2, CheckCircle2, AlertCircle, Video, CheckCircle, Clock } from "lucide-react";
...
export default function MediaPage() {
  ...
  const getCardsData = (): DashboardCardConfig[] => {
    if (activeTab === 0) {
      return [
        { title: "Total Peminjaman", value: data.length, description: "Semua Permintaan", icon: <Video className="w-6 h-6" />, colorTheme: "slate" },
        { title: "Tertunda", value: data.filter((d: any) => d.status === 'Tertunda').length, description: "Menunggu Persetujuan", icon: <Clock className="w-6 h-6" />, colorTheme: "amber" },
        { title: "Disetujui / Selesai", value: data.filter((d: any) => d.status === 'Disetujui' || d.status === 'Selesai').length, description: "Sudah Diproses", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "indigo" },
      ];
    } else {
      return [
        { title: "Total Laporan", value: data.length, description: "Seluruh Tiket", icon: <Video className="w-6 h-6" />, colorTheme: "slate" },
        { title: "Belum Selesai", value: data.filter((d: any) => d.status === 'Buka' || d.status === 'Dalam Proses').length, description: "Sedang Ditangani", icon: <Clock className="w-6 h-6" />, colorTheme: "rose" },
        { title: "Selesai", value: data.filter((d: any) => d.status === 'Selesai').length, description: "Gangguan Teratasi", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "emerald" },
      ];
    }
  };
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Seksi Media & Lab</h1>
