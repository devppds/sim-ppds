...
import { Plus, Search, RefreshCw, X, Pencil, Trash2, CheckCircle2, AlertCircle, Music, Clock, CheckCircle } from "lucide-react";
...
export default function JamiyyahPage() {
  ...
  const getCardsData = (): DashboardCardConfig[] => {
    if (activeTab === 0) {
      return [
        { title: "Total Kegiatan", value: data.length, description: "Semua Acara", icon: <Music className="w-6 h-6" />, colorTheme: "indigo" },
        { title: "Akan Datang", value: data.filter((d: any) => d.status === 'Konsep' || d.status === 'Dipublikasikan').length, description: "Persiapan & Terjadwal", icon: <Clock className="w-6 h-6" />, colorTheme: "blue" },
        { title: "Selesai", value: data.filter((d: any) => d.status === 'Selesai').length, description: "Acara Berhasil", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "emerald" },
      ];
    } else {
      return [
        { title: "Total Aset", value: data.length, description: "Seluruh Inventaris", icon: <Music className="w-6 h-6" />, colorTheme: "indigo" },
        { title: "Kondisi Baik", value: data.filter((d: any) => d.condition === 'Baik').length, description: "Siap Digunakan", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "emerald" },
        { title: "Rusak / Hilang", value: data.filter((d: any) => d.condition === 'Rusak' || d.condition === 'Hilang').length, description: "Perlu Perhatian", icon: <AlertCircle className="w-6 h-6" />, colorTheme: "rose" },
      ];
    }
  };
