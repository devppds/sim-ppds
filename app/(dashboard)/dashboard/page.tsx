...
export default function DashboardPage() {
  ...
  return (
    <>
      ...
      {/* Incomplete Santri Warning for Sekretaris */}
      {isSekretaris && data?.incomplete_santri && data.incomplete_santri.total > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-3xl p-6 mb-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-amber-800 font-extrabold flex items-center gap-2">
                <AlertCircle className="w-5 h-5" /> Data Santri Kurang Lengkap ({data.incomplete_santri.total} Santri)
              </h3>
              <p className="text-amber-700/80 text-xs font-medium mt-1">
                Data NIK, Tempat/Tanggal Lahir, atau Data Wali dari santri berikut masih kosong. Segera lengkapi!
              </p>
            </div>
            <button 
              onClick={() => router.push('/santri')} 
              className="hidden sm:block text-xs font-bold bg-amber-100 hover:bg-amber-200 text-amber-800 px-4 py-2 rounded-xl transition-colors"
            >
              Lihat Semua Santri
            </button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
            {data.incomplete_santri.list.map((s: any) => (
              <div key={s.id} className="bg-white p-4 rounded-2xl shadow-sm border border-amber-100 flex flex-col justify-between h-full gap-3">
                <div>
                  <p className="font-bold text-slate-800 text-sm line-clamp-1">{s.name}</p>
                  <p className="text-[10px] font-bold text-slate-500 mt-0.5 uppercase tracking-wider">{s.kelas} • {s.asrama || '-'}</p>
                </div>
                <button 
                  onClick={() => router.push(`/santri?id=${s.id}`)}
                  className="w-full text-center px-3 py-2 bg-amber-100/50 hover:bg-amber-500 hover:text-white text-amber-700 text-xs font-black rounded-xl transition-all"
                >
                  LENGKAPI DATA
                </button>
              </div>
