...
import { Plus, Search, RefreshCw, X, Pencil, CheckCircle2, AlertCircle, Trash2, CheckCircle, Clock } from "lucide-react";
...
export default function KebersihanPage() {
  ...
  const getCardsData = (): DashboardCardConfig[] => {
    return [
      { title: "Total Cek", value: data.length, description: "Semua Laporan", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "teal" },
      { title: "Kotor / Peringatan", value: data.filter((d: any) => d.status === 'Kotor' || d.status === 'Peringatan').length, description: "Perlu Dibersihkan", icon: <AlertCircle className="w-6 h-6" />, colorTheme: "rose" },
      { title: "Bersih", value: data.filter((d: any) => d.status === 'Bersih').length, description: "Kondisi Optimal", icon: <CheckCircle2 className="w-6 h-6" />, colorTheme: "emerald" },
    ];
  };
