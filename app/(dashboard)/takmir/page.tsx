...
export default function TakmirPage() {
  ...
  const getCardsData = (): DashboardCardConfig[] => {
    if (activeTab === 0) {
      return [
        { title: "Total Jadwal", value: data.length, description: "Keseluruhan di tab ini", icon: <MoonStar className="w-6 h-6" />, colorTheme: "emerald" },
        { title: "Jadwal Imam", value: data.filter((d: any) => d.role === 'Imam').length, description: "Petugas Imam", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "teal" },
        { title: "Jadwal Muadzin", value: data.filter((d: any) => d.role === 'Muadzin').length, description: "Petugas Muadzin", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "blue" },
      ];
    } else {
      return [
        { title: "Total Booking", value: data.length, description: "Semua Pengajuan", icon: <MoonStar className="w-6 h-6" />, colorTheme: "emerald" },
        ...
        { title: "Disetujui", value: data.filter((d: any) => d.status === 'Disetujui').length, description: "Telah Disetujui", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "teal" },
      ];
    }
  };
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Takmir Masjid DS</h1>
