...
import { Plus, Search, RefreshCw, X, Pencil, Trash2, CheckCircle2, AlertCircle, Hammer, Clock, CheckCircle } from "lucide-react";
...
export default function PembangunanPage() {
  ...
  const getCardsData = (): DashboardCardConfig[] => {
    return [
      { title: "Total Proyek", value: data.length, description: "Keseluruhan Proyek", icon: <Hammer className="w-6 h-6" />, colorTheme: "amber" },
      { title: "Dalam Proses", value: data.filter((d: any) => d.status === 'Dalam Proses' || d.status === 'Diajukan').length, description: "Sedang Dikerjakan", icon: <Clock className="w-6 h-6" />, colorTheme: "orange" },
      { title: "Selesai", value: data.filter((d: any) => d.status === 'Selesai').length, description: "Proyek Selesai", icon: <CheckCircle className="w-6 h-6" />, colorTheme: "emerald" },
    ];
  };
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Seksi Pembangunan</h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="p-4">
          <DataTable
            ...
            sortOptions={[
              { label: "Terbaru", value: "id-desc", sortFn: (a: any, b: any) => b.id - a.id },
              { label: "Terlama", value: "id-asc", sortFn: (a: any, b: any) => a.id - b.id }
            ]}
