...
export const getSearchResults = async (c: Context<{ Bindings: Env }>) => {
  try {
    ...
    const ustadzSearch = await c.env.DB.prepare(`
      SELECT 'ustadz' as type, id, name, photo_url, (CASE WHEN bidang IS NULL OR bidang = '' THEN jabatan WHEN jabatan IN ('Ketua', 'Sekretaris', 'Bendahara') THEN bidang ELSE jabatan || ' (' || bidang || ')' END) as info
      FROM ustadz
