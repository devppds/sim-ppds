...
export async function POST(request: Request) {
  try {
    ...
    if (!username || !password) {
      return c.json({ success: false, error: "Username and password required" }, { status: 400 });
    }
    ...
    if (!env || !env.DB) {
       return c.json({ success: false, error: "Database not connected" }, { status: 500 });
    }
    ...
    if (!user) {
      return c.json({ success: false, error: "Username atau Password salah" }, { status: 401 });
    }

    if (user.status !== "Aktif") {
      return c.json({ success: false, error: "Akun dinonaktifkan" }, { status: 403 });
    }
    ...
    return c.json({ success: false, error: "Authentication failed" }, { status: 500 });
  }
}
