...
export async function GET(request: Request) {
  try {
    ...
    if (entryMonth) {
      ...
      if (newStudentRow) return c.json({ success: true, amount: newStudentRow.amount });
    }
    ...
    return c.json({ success: false, error: "Gagal mengambil tarif" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return c.json({ success: true, message: "Konfigurasi disimpan" });
  } catch (error) {
    console.error("Config POST Error:", error);
    return c.json({ success: false, error: "Gagal menyimpan konfigurasi" }, { status: 500 });
  }
}
