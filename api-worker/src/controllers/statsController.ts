...
export const getSeksiStats = async (c: Context<{ Bindings: Env }>) => {
  try {
    ...
    if (membersPattern) {
      const { results } = await c.env.DB.prepare(
        "SELECT id, name, phone, (CASE WHEN bidang IS NULL OR bidang = '' THEN jabatan WHEN jabatan IN ('Ketua', 'Sekretaris', 'Bendahara') THEN bidang ELSE jabatan || ' (' || bidang || ')' END) as jabatan, kamar, photo_url, status FROM ustadz WHERE (jabatan LIKE ? OR jabatan_tambahan LIKE ?) AND status = 'Aktif' ORDER BY name ASC"
